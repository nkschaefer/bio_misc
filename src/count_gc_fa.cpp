 ...
 */
void help(int code){
    fprintf(stderr, "count_gc_fa [OPTIONS]\n");
    fprintf(stderr, "Given a FASTA file, prints the percent GC across the file\n");
...
int main(int argc, char *argv[]) {    
    ...
    gzFile f_fp; 
    if (fasta == "-"){
        f_fp = gzdopen(fileno(stdin), "r");
    }
    else{ 
        f_fp = gzopen(fasta.c_str(), "r");
    ...
    while ((progress = kseq_read(seq)) >= 0){
        ...
        for (int i = 0; i < seq->seq.l; ++i){
            if (seq->seq.s[i] == 'g' || seq->seq.s[i] == 'G' || 
                seq->seq.s[i] == 'c' || seq->seq.s[i] == 'C'){
                gc++;
                tot++;
            }
            else if (seq->seq.s[i] == 'a' || seq->seq.s[i] == 'A' ||
                seq->seq.s[i] == 't' || seq->seq.s[i] == 'T'){
                tot++;
            }
        }
    }
